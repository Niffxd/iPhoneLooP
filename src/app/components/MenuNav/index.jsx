...
export default function MenuNav () {
  const handleMenu = () => {
    ...
    if (!document.getElementById('menu-links').classList.value.includes(`${style['show-menu-links']}`)) { // @ts-expect-error: 'Object is possibly null'
    ...
    <div className={style.navbar_links_container}>
        ...
        <div id='menu-links' className={style.menu_links}>
          <ul>
            {
              links.map(item => {
                return item === 'Home' || item === 'Services' || item === 'Calendar'
                  ? (
                      <Link className={style.link} key={item} href={item === 'Home' ? '/' : `/${item.toLowerCase().split(' ').join('-')}`} onClick={handleMenu}>
                        <li>{item}</li>
                      </Link>
                    )
                  : '';
          ...
          <Link className={style.link_to_book} href='/system.html' target='_blank'>
            Book
          </Link>
        </div>
      </div>
